...
class BallGame1DChoiceState(BallGame1DChoice):
    ...
    def __init__(self):
        ...
        self._grandularity = 20 # This should be an even number
        self._action_dimension=1
        self._range = 1.0

    
    def getState(self):
        state = np.zeros(self._grandularity+1)
        ...
        for i in range(int(self._choices)):
            ...
            delta = (delta)/(self._range/(self._grandularity/2.0))
            # print "Second delta: " + str(delta)
            index = int(delta+(self._grandularity/2)) + self._action_dimension
    ...
    def setTargetChoice(self, i):
        ...
        _loc = np.linspace(-self._range, self._range, self._grandularity)[i]
