...
class NeuralNet(object):
    ...
    def __init__(self, input, n_in, n_out):
        ...
        q_val = T.max(py_x)
        q_func = T.mean((self.model(State, self._w_h, self._b_h, self._w_o, self._b_o))[T.arange(Action.shape[0]), Action.reshape((-1,))].reshape((-1, 1)))
        ...
        delta = ((Reward + (self._discount_factor * 
                            ...
                            (self.model(State, self._w_h, self._b_h, self._w_o, self._b_o))[T.arange(Action.shape[0]), Action.reshape((-1,))].reshape((-1, 1)))
